...
p_rETR2 <-  ggplot(d, aes(y=rETR, x=PAR, colour=treatment,group=treatment)) + 
  ...
  xlab(bquote('Irradiance �mol photons '~ m^-2~s^-1*''))+ylab('Relative ETR')
...
p_effective <-  ggplot(d, aes(y=effective, x=PAR, colour=treatment,group=treatment)) + 
  ...
  xlab(bquote('Irradiance �mol photons '~ m^-2~s^-1*''))+ylab('Effective quantum yield')
#labs(x=expression(paste("Âµmol photons ", m^{-2})),y='Effective quantum yield')
p_effective

p_NPQ <-  ggplot(d, aes(y=NPQ, x=PAR, colour=treatment,group=treatment)) + 
  #  geom_line(size=0.5)+#geom_point(aes(shape=treatment, color=treatment))+
  # geom_errorbar(aes(ymin = mean_eff-se, ymax= mean_eff+se),  
  #              width=.5, position=position_dodge(0.05))+
  #geom_point(aes(colour= treatment),size=2,alpha=0.5)+
  ...
  theme(legend.position="none",
        ...
        axis.text=element_text(size=10),
        axis.text.x = element_text(size=10))+
  xlab(bquote('Irradiance �mol photons '~ m^-2~s^-1*''))+ylab('Non-photochemical quenching')
...
ggsave("NPQ.tiff", plot = p_NPQ, width = 9 , height = 8.5 ,dpi=600, 
       units = "cm")
ggsave("Effective_quantum_yield.tiff", plot = p_effective, width = 9 , height = 8.5 ,dpi=600, 
       units = "cm")
ggsave("rETR.tiff", plot = p_rETR2, width = 9 , height = 8.5 ,dpi=600, 
       units = "cm")
ggsave("for_legend_only.tiff", plot = p_NPQ2, width = 9 , height = 8.5 ,dpi=600, 
       units = "cm")
#########not using this
